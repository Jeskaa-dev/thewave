class FetchDatasJob < ApplicationJob
  queue_as :default

  def perform(user)
    Rails.logger.info "Starting fetch job for user #{user.id}..."

    puts 'Fetch en cours...'

    fetch_github_commits
    Rails.logger.info "Commits fetched successfully for user #{user.id}"

    create_training_plans
    Rails.logger.info "Training plan created with ID: #{training_plan&.id}"

    puts "OK j'ai terminé"
  end
